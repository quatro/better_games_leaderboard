class Athlete < ApplicationRecord
  ...
  def store_json_scores(json)
    json.each_with_index do |score,i|
      if score
        rs = regional_scores.where(year:2019, workout:i+1).first
        ...
        hash = {scoredisplay:scoredisplay, year:2019, workout:i+1, score: scoredisplay, minutes:minutes, seconds:seconds, fraction_seconds: fraction_seconds, cap_count:cap_count, is_wd:is_wd }
